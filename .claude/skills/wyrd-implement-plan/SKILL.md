...
| Material plan revision | `plan-readiness-reviewer` | Opus |
...
5. dispatch a `plan-readiness-reviewer` agent in advisory mode for every material
   revision;
...
- Repair material plan decisions autonomously and obtain advisory
  `plan-readiness-reviewer` approval before resuming.
