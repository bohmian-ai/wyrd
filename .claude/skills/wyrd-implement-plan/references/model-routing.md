...
| Material plan revision | `plan-readiness-reviewer` | Opus | Gates architecture and executability of orchestrator-authored revisions |
...
5. **Task or plan revision.** The task itself is wrong. Revise it, obtain
   `ADVISORY_APPROVE` from `plan-readiness-reviewer`, and restart the cycle.
