...
- `wyrd-spec` is IO-free, async-free, and foundational. It is PyO3-free
  except for foundation pointer types that users must construct directly
  (today: `CardRef`, gated behind a `python` feature). Specs, schemas,
  validators, the error catalog, and identity newtypes stay PyO3-free.
