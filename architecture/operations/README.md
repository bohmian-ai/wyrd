...
- loaded authentication, signing, peer, and audit-anchor keys;
...
Readiness is role-specific. A replica that can serve registry traffic but
cannot satisfy its enabled Scribe role is not ready for Scribe traffic. The
...
listeners, and one entry for each selected Bifrost runtime role. The top-level
...
| Public serving | Gateway/listener transport, token verifier and JWKS state, permission resolver, policy decision point, canonical audit writer, external anchor publisher, route contracts and request governors |
