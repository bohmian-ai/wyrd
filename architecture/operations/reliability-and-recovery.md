...
- external audit-anchor lag and verification outcome; and
...
- external audit checkpoints and their verification keys;
- encrypted signing, peer, Source, storage, and audit-anchor key backups under
...
5. Verify external audit checkpoints and identify the exact unanchored suffix.
...
3. Preserve logs, traces, audit chains, external checkpoints, database WAL,
   object versions, Scribe volumes, task/lease state, and deployment
...
Security incidents additionally rotate affected signing, peer, Source,
storage, and audit-anchor credentials; advance authorization epochs; and verify
that no retired material remains accepted.
...
- audit-chain and external-anchor verification;
