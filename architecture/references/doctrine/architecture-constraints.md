...
- `sdks/wyrd-sdk-rust`, `sdks/wyrd-sdk-python`, and `sdks/wyrd-sdk-ts`
  consume `wyrd-client`, the sole shared Rust client implementation and
  SDK-facing surface. Language-specific behavior must
  be earned by its runtime boundary and cannot duplicate transport or durable
  semantics.
- Existing owner crates may retain optional `python` features as migration
  state. Only `wyrd-sdk-python` enables them; Rust and TypeScript SDKs do not.
  New or materially relocated Python logic belongs in the Python SDK.
- Bifrost is exposed through `wyrd_client::Bifrost` over the shared client HTTP
  and gRPC transport. Gate, Scribe, Oracle, and Forge remain server-owned and are never
  client types.
