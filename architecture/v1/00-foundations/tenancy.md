...
| `wyrd_platform_admin` | Audited cross-tenant support and platform operations. Has `BYPASSRLS`; optional or break-glass in dedicated deployments. |

Deployment modes share the same code path:

| Mode | Tenant catalog | Platform-admin posture |
|---|---|---|
| Self-hosted | Usually one `platform.tenants` row, with more allowed for internal multi-tenancy. | Optional operational role. |
| Cloud multi-tenant | One shared cluster with many tenant rows. | Required small audited pool. |
| Cloud dedicated | One customer cluster with one tenant row. | Break-glass credential only; no silent fallback to `wyrd_app`. |
