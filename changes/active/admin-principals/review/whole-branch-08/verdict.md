...
The revision-12 core authentication architecture is implemented at its principal
...
table identities at admission. Seven independently validated defects remain
after the human dispositions of `FIND-admin-principals-R8-1` and
`FIND-admin-principals-R8-4`.
After review, the user directed that the two rejected pre-existing delegation
defects also be corrected as credential-exchange security work and explicitly
approved specification revision 13. `REQ-012c`, `INV-013a`, and `AC-020` now
require authority attenuation and durable allow/deny audit without changing the
delegation-chain representation, depth, or emit Card scope.

Remediation task:
`changes/active/admin-principals/review/whole-branch-08/TASK-001-008-R8-close-validated-findings.md`.
The task incorporates approved revision 13 and is ready for `$wyrd-implement`.
...
| Revision 13 `REQ-012c`, `INV-013a`, `AC-020`: delegation attenuates authority and durably audits every decision | Current exchange checks `delegation:issue` but then mints the target's complete current permissions; a denied check returns before canonical audit | **FAIL** — approved post-review credential-exchange security work |
...
| Tenant isolation | `TenantConn` and forced RLS remain the owners | **FAIL** — redundant tenant predicates remain |
...
| Revision-12 non-goals and bounded scope | No replacement auth abstraction was added; the audit-publisher `NOWAIT` change is retained as a required canonical-audit availability fix | PASS |
...
| Ponytail validation | COMPLETE | Nine proposed retained roots; the human authority subsequently rejected `FIND-admin-principals-R8-1` and brought the two delegation defects into approved revision 13 |
...
## Human disposition of `FIND-admin-principals-R8-1`

The audit-publisher `FOR UPDATE NOWAIT` change in `67b4d0ba` is required to
prevent one locked tenant from stalling publication for every tenant. Keep its
production change and replay proof. `FIND-admin-principals-R8-1` is rejected
after validation and is not remediation work.

## Human disposition of `FIND-admin-principals-R8-4`

Wyrd has not shipped and supports no predecessor production audit tables or
hash chains. The proposed compatibility migration would protect an unreachable
state and add machinery before the first release. Keep the new audit schema and
hash format as the sole format; `FIND-admin-principals-R8-4` is rejected after
validation and is not remediation work.

## Approved post-review delegation requirements

Wave 2 correctly rejected `AUTH-R8-1` and `AUTH-R8-2` under revision 12 because
they predated the base and delegation semantics were then an explicit non-goal.
The user subsequently brought them into scope and approved revision 13. They
are therefore required specification work, not retroactively validated Wave 2
findings:

- A delegated token's `permissions` must be the semantic intersection of the
  caller token's verified authority and the target principal's current grants,
  retaining the narrower overlapping scope.
- Every evaluated `delegation:issue` allow or deny must commit exactly one
  canonical audit decision before the response; later subject or issuance
  failure retains an allowed no-effect decision, and audit failure issues no
  token.
