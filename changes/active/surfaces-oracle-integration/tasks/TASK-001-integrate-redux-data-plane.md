...
## Scope Amendment — audit at the authorization boundary

Folded into TASK-001 closeout. This supersedes the audit portions of the
status amendment above: the interim `CorrelationPolicy::Observation` change and
the `audit_principal_id` rename are withdrawn.

This changes decisions fixed in `architecture/bifrost-design.md:454-463` —
audit fires at authorization boundaries only, `vala.audit_outbox` becomes
`vala.audit_staging` with watermark semantics, and `vala.system.audit_log`
deviates to daily partitioning. Those revisions require human approval.
...
| `decision` + `result` | Three reachable states, two booleans. Merge to `outcome` ∈ `{allowed_success, allowed_failure, denied}` |
...
`event_time_past_window_secs` defaults to `None` (`wyrd-server/src/config.rs:1547`),
so a backlog replays unbounded by default. A deployment that configures a past
window must exempt the audit path; record this in the design doc.
