...
impl IssuingKey {
    ...
    /// Mint a refresh token for a user, service, or agent principal from an
    /// explicit instant.
    ///
    ...
    ///
    /// # Errors
    /// Returns [`IssueError::InvalidPrincipalKind`] for
    /// [`GATEWAY_CAPTURE_PRINCIPAL`], and an error when TTL is invalid or
    /// signing fails.
    #[tracing::instrument(
        level = "debug",
        skip(self),
