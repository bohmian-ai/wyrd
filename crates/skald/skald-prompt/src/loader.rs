...
/// Load a `PromptSpec` file (native or declarative envelope) and return the
/// prompt builder wrapper.
///
/// Declarative dispatch (when the spec has a `provider` key) is handled inside
/// `PromptSpec`'s custom `Deserialize` — no silent fallback here.
