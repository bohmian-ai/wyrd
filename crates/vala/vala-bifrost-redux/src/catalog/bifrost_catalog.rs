...
/// One sealed manifest projected into the exclusion a memory provider may apply.
///
/// The two fields answer different questions and a provider needs both. The
/// cursor is the boundary below which the stream is completely published, so
/// everything at or under it is owned by objects no matter which bucket
/// produced it. The ranges name the individual published generations, including
/// the ones above that boundary, so a generation an object already holds is not
/// served from memory a second time while it waits to retire.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct PersistedWalCut {
    /// Inclusive boundary below which the stream is completely published.
    pub persisted_cursor: u64,
    /// Inclusive bounds of every published generation, in ascending order.
    pub persisted_ranges: Vec<PersistedWalRange>,
}

/// Projects a sealed manifest into one published cursor and its generation bounds.
///
/// A Scribe node numbers WAL records from one node-global counter while it
/// seals generations per `(table, partition)` bucket, so one generation's
/// inclusive `[min, max]` bounds routinely span records that belong to a
/// different bucket. Two published generations may therefore overlap without
/// either owning the other's records, and the cursor is deliberately the
/// *completely covered* prefix rather than the highest published bound: a
/// record above it may still be sitting unpublished inside another generation's
/// span, and excluding it would hide an acknowledged row.
///
/// # Errors
///
/// Returns a metadata mismatch when a generation carries negative or inverted
/// bounds, a duplicate ordinal, non-contiguous ordinals, or a bound outside the
/// durable signed domain.
pub fn project_persisted_wal_cut(
    rows: &[vala_sql::row_types::file_list::HotFileRow],
) -> Result<PersistedWalCut, BifrostCatalogError> {
    ...
    for range in &ranges {
        let contiguous = match covered {
            None => range.start_lsn == 0,
            Some(end) => range.start_lsn <= end.saturating_add(1),
        };
        if !contiguous {
            break;
        }
        covered = Some(covered.map_or(range.end_lsn, |end| end.max(range.end_lsn)));
    }
    Ok(PersistedWalCut {
        persisted_cursor: covered.unwrap_or(0),
...
/// Unit coverage for the published-prefix projection over a sealed manifest.
#[cfg(test)]
mod persisted_prefix_tests {
    use super::project_persisted_wal_cut;

    /// Builds one sealed-manifest row for a generation's inclusive WAL bounds.
    ///
    /// Only the fields the prefix projection reads carry meaning here; every
    /// other column is filled with a fixed value so the cases differ solely in
    /// stream identity, bounds, and ordinal.
    fn sealed_row(
        wal_lsn_min: i64,
        wal_lsn_max: i64,
        file_ordinal: i16,
        ...
        vala_sql::row_types::file_list::HotFileRow {
            id: uuid::Uuid::nil(),
            data_tenant_id: uuid::Uuid::nil(),
            namespace: "datasets".to_owned(),
            table_name: "events".to_owned(),
            file_path: format!("hot/{wal_lsn_min}-{wal_lsn_max}-{file_ordinal}.parquet"),
            file_ordinal,
            file_checksum: None,
            file_size: 1,
            row_count: 1,
            partition_granularity: "hour".to_owned(),
            partition_start: chrono::DateTime::UNIX_EPOCH,
            compacted: false,
    ...
    /// Generations whose bounds interleave still yield their covered prefix.
    ///
    /// A node numbers WAL records globally while sealing per partition, so a
    /// generation covering records 0 and 2 legitimately spans the record 1 that
    /// a second partition's generation owns. Both are published, so every record
    /// through 2 is published and the cursor is 2.
    #[test]
    fn interleaved_generations_project_their_covered_prefix() {
        let rows = vec![sealed_row(0, 2, 0), sealed_row(1, 1, 0)];

        assert_eq!(
            project_persisted_wal_cut(&rows)
                .expect("an interleaved manifest projects")
                .persisted_cursor,
            2
        );
    }

    /// A gap ends the prefix, leaving later generations to the memory provider.
    ///
    /// Nothing published covers record 3, so the records above it may not be
    /// excluded from memory even though a published generation names them.
    #[test]
    fn a_gap_ends_the_published_prefix() {
        let rows = vec![sealed_row(0, 2, 0), sealed_row(4, 6, 0)];

        let cut = project_persisted_wal_cut(&rows).expect("a gapped manifest projects");

        assert_eq!(cut.persisted_cursor, 2);
        assert_eq!(
            cut.persisted_ranges.len(),
            2,
            "a generation above the covered prefix is still named as published"
        );
    }

    /// A stream that never published its first record has no prefix at all.
    #[test]
    fn a_stream_without_its_first_record_has_no_prefix() {
        let rows = vec![sealed_row(1, 3, 0)];

        assert_eq!(
            project_persisted_wal_cut(&rows)
                .expect("a manifest without a first record projects")
                .persisted_cursor,
            0
        );
    }

    /// One generation's object set must be a contiguous ordinal run.
    ///
    /// A missing ordinal means the manifest lost part of a generation, which
    /// makes its bounds unusable as a published-coverage statement.
    #[test]
    fn a_generation_with_a_missing_ordinal_is_refused() {
        let rows = vec![sealed_row(0, 2, 0), sealed_row(0, 2, 2)];

        assert!(project_persisted_wal_cut(&rows).is_err());
    }
}
