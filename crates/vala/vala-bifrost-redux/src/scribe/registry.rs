...
impl ScribeHeartbeat {
    ...
    /// [`live_scribes`].
    #[must_use]
    pub fn start(pool: OperatorPool, node_id: NodeId, interval: Duration) -> Self {
        let handle = tokio::spawn(async move {
            ...
            loop {
                ticker.tick().await;
                if let Err(err) = heartbeat_tick(&pool, node_id).await {
                    tracing::warn!(
                        %node_id,
                        error = %err,
...
pub async fn live_scribes(pool: &OperatorPool) -> Result<Vec<LiveScribe>, ScribeError> {
