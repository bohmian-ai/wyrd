//! Cross-table domain derivations (Task 05b): pure, IO-free transforms that turn
...
pub trait DomainDerivation: Send + Sync + 'static {
    ...
    ///
    /// # Errors
    /// Returns [`BifrostError`] when the source batch is missing a required column
    /// or has an unexpected physical type.
    fn derive(
        &self,
        source: &RecordBatch,
        source_wyrd_batch_id: [u8; 16],
    ) -> Result<Vec<DerivedBatch>, BifrostError>;
...
impl<'a> SpanColumns<'a> {
    fn from_batch(batch: &'a RecordBatch) -> Result<Self, BifrostError> {
        Ok(Self {
    ...
    fn tenant(&self, i: usize) -> Result<DataTenantId, BifrostError> {
        let raw = self.data_tenant_id.value(i);
        let uuid = Uuid::parse_str(raw).map_err(|e| {
            BifrostError::Internal(format!("spans.{DATA_TENANT_ID} not a uuid: {e}"))
        })?;
        DataTenantId::try_from(uuid)
            .map_err(|e| BifrostError::Internal(format!("spans.{DATA_TENANT_ID} invalid: {e}")))
    }
...
impl EmbeddingRow {
    ...
    fn to_batch(rows: &[Self]) -> Result<RecordBatch, BifrostError> {
        ...
        .map_err(|e| BifrostError::Internal(format!("build genai.embeddings batch: {e}")))
    }
}
...
impl ToolCallRow {
    ...
    fn to_batch(rows: &[Self]) -> Result<RecordBatch, BifrostError> {
        ...
        .map_err(|e| BifrostError::Internal(format!("build genai.tool_calls batch: {e}")))
    }
}
...
impl MemoryRow {
    ...
    fn to_batch(rows: &[Self]) -> Result<RecordBatch, BifrostError> {
        ...
        .map_err(|e| BifrostError::Internal(format!("build genai.memory batch: {e}")))
    }
}
...
fn fixed_bin<'a>(
    batch: &'a RecordBatch,
    name: &str,
) -> Result<&'a FixedSizeBinaryArray, BifrostError> {
    batch
        .column_by_name(name)
        .ok_or_else(|| BifrostError::Internal(format!("spans missing {name}")))?
        .as_any()
        .downcast_ref::<FixedSizeBinaryArray>()
        .ok_or_else(|| BifrostError::Internal(format!("spans {name} not FixedSizeBinary")))
}
...
fn ts_us<'a>(
    batch: &'a RecordBatch,
    name: &str,
) -> Result<&'a TimestampMicrosecondArray, BifrostError> {
    batch
        .column_by_name(name)
        .ok_or_else(|| BifrostError::Internal(format!("spans missing {name}")))?
        .as_any()
        .downcast_ref::<TimestampMicrosecondArray>()
        .ok_or_else(|| BifrostError::Internal(format!("spans {name} not TimestampMicrosecond")))
}

fn int64<'a>(batch: &'a RecordBatch, name: &str) -> Result<&'a Int64Array, BifrostError> {
    batch
        .column_by_name(name)
        .ok_or_else(|| BifrostError::Internal(format!("spans missing {name}")))?
        .as_any()
        .downcast_ref::<Int64Array>()
        .ok_or_else(|| BifrostError::Internal(format!("spans {name} not Int64")))
}

fn string<'a>(batch: &'a RecordBatch, name: &str) -> Result<&'a StringArray, BifrostError> {
    batch
        .column_by_name(name)
        .ok_or_else(|| BifrostError::Internal(format!("spans missing {name}")))?
        .as_any()
