mod pg_tests {
    ...
    async fn catalog_from_fixture(fixture: &PgFixture) -> vala_bifrost::WyrdCatalog {
        let tmp = tempfile::tempdir().unwrap();
        let pool = Arc::new(fixture.app_pool().clone());
        let backend = wyrd_storage::settings::BackendConfig::Local {
            root: tmp.path().to_path_buf(),
        };
        vala_bifrost::WyrdCatalog::new(&fixture.catalog_uri(), &backend, pool, None)
            .await
            .unwrap()
    }
    ...
    async fn run_ingest_rbac_denial_rejects_principal_without_record_write() {
        ...
        let catalog = catalog_from_fixture(&fixture).await;
