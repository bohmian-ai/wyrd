...
use vala_sdk::{Bifrost, BifrostIngestSink, ClientScope, IngestTransport, SinkKind, observe};
...
fn identical_key_reuses_one_pooled_producer() {
    let scope = ClientScope::from_config(&config_with_key("http://x", "secret")).expect("scope");
...
fn insert_propagates_queue_full() {
    ...
    for i in 0..40 {
        ...
        if let Err(err) = bifrost.insert(SinkKind::Record, "ns.tbl", &schema, payload, card(), None)
        {
            assert_eq!(
                err.code(),
...
fn schema_from_json_schema_builds_arrow_schema() {
    ...
    let schema = vala_sdk::schema_from_json_schema(&json).expect("schema builds");
