mod pg_tests {
    ...
    mod drift_alerts {
        ...
        async fn setup() -> (
            wyrd_dev_fixtures::pg::PgFixture,
            sqlx::PgPool,
            DataTenantId,
            DataTenantId,
        ) {
            ...
            fixture
                .seed_additional_tenant_with_uuid(a, &format!("test-{}", a.as_uuid().simple()))
                .await
                .unwrap();
            fixture
                .seed_additional_tenant_with_uuid(b, &format!("test-{}", b.as_uuid().simple()))
                .await
                .unwrap();
            (fixture, superuser, a, b)
        }

        #[tokio::test]
        async fn upsert_creates_active_row() {
            let (fixture, superuser, tenant_a, _) = setup().await;
        ...
        async fn rls_isolation_tenant_b_cannot_see_tenant_a_alerts() {
            let (fixture, _superuser, tenant_a, tenant_b) = setup().await;
            let drift_ref = card_ref();

            let mut conn_a = TenantConn::acquire(fixture.app_pool(), tenant_a)
                .await
                .unwrap();
            upsert_drift_alert(
                &mut conn_a,
