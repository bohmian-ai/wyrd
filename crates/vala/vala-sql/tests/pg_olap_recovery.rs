mod pg_tests {
    ...
    async fn setup() -> (PgFixture, PgPool, DataTenantId, [u8; 16], [u8; 16]) {
        ...
        (fixture, superuser, tenant, table_uid, batch_id)
    }

    #[tokio::test]
    async fn aborted_state_accepted() {
        let (fixture, _superuser, tenant, table_uid, batch_id) = setup().await;
        ...
        let mut conn = vala_sql::TenantConn::acquire(fixture.app_pool(), tenant)
            ...
            .unwrap();
        let row =
            vala_sql::queries::olap_catalog::lookup_idempotent(&mut conn, &table_uid, &batch_id)
                .await
                .unwrap()
    ...
    async fn audit_row_roundtrips_with_byte_ids() {
        let (fixture, superuser, tenant, table_uid, batch_id) = setup().await;
    ...
    async fn renew_fence_returns_false_after_claim() {
        let (fixture, superuser, tenant, table_uid, _) = setup().await;
        let batch_id = *uuid::Uuid::now_v7().as_bytes();
    ...
    async fn scan_failed_leaves_precommit_and_allows_retry() {
        ...
        let (state, attempts, last_error, fencing_token): (
            String,
            i32,
            Option<String>,
            Option<i64>,
