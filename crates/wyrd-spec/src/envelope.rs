...
impl Spec {
    ...
    /// Visit every typed `CardRef` field in this spec.
    pub fn walk_refs<V: ReferenceSlotVisitor>(&self, visitor: &mut V) {
        match self {
            Self::Data(spec) => {
                ...
                match &spec.interface {
                    DataInterface::Image(meta) => visit_ref(meta.manifest_ref.as_ref(), visitor),
                    DataInterface::Text(meta) => visit_ref(meta.manifest_ref.as_ref(), visitor),
                    _ => {}
                }
            }
            Self::Model(spec) => visit_refs(&spec.card_refs, visitor),
            Self::Experiment(spec) => {
                visit_refs(&spec.target_refs, visitor);
                visit_refs(&spec.card_refs, visitor);
            }
            Self::Prompt(_) => {}
            ...
            Self::Policy(_) => {}
            Self::Mcp(spec) => visit_refs(&spec.tool_refs, visitor),
            Self::Audit(spec) => {
                visit_refs(&spec.subject_refs, visitor);
                visit_refs(&spec.policy_refs, visitor);
                visit_refs(&spec.evidence_refs, visitor);
            }
            ...
            Self::Operator(spec) => {
                ...
                visit_refs(&spec.post_invoke, visitor);
            }
            Self::Source(_) => {}
        }
    }

    /// Mutably visit every typed `CardRef` field in this spec.
    pub fn walk_refs_mut<V: ReferenceSlotVisitor>(&mut self, visitor: &mut V) {
        match self {
            Self::Data(spec) => {
                ...
                match &mut spec.interface {
                    DataInterface::Image(meta) => {
                        visit_ref_mut(meta.manifest_ref.as_mut(), visitor)
                    }
                    DataInterface::Text(meta) => visit_ref_mut(meta.manifest_ref.as_mut(), visitor),
                    _ => {}
                }
            }
            Self::Model(spec) => visit_refs_mut(&mut spec.card_refs, visitor),
            Self::Experiment(spec) => {
                visit_refs_mut(&mut spec.target_refs, visitor);
                visit_refs_mut(&mut spec.card_refs, visitor);
            }
            Self::Prompt(_) => {}
            ...
            Self::Policy(_) => {}
            Self::Mcp(spec) => visit_refs_mut(&mut spec.tool_refs, visitor),
            Self::Audit(spec) => {
                visit_refs_mut(&mut spec.subject_refs, visitor);
                visit_refs_mut(&mut spec.policy_refs, visitor);
                visit_refs_mut(&mut spec.evidence_refs, visitor);
            }
            ...
            Self::Operator(spec) => {
                ...
                visit_refs_mut(&mut spec.post_invoke, visitor);
            }
            Self::Source(_) => {}
        }
    }
...
/// Visitor over the typed reference slots in a Card spec.
pub trait ReferenceSlotVisitor {
    /// Observe one immutable CardRef slot.
    fn visit_ref(&mut self, card_ref: &CardRef);
    /// Mutate one CardRef slot.
...
fn visit_trigger_source<V: ReferenceSlotVisitor>(
    source: &crate::card::trigger::TriggerSource,
    visitor: &mut V,
) {
    match source {
...
fn visit_trigger_source_mut<V: ReferenceSlotVisitor>(
    source: &mut crate::card::trigger::TriggerSource,
    visitor: &mut V,
) {
    match source {
        crate::card::trigger::TriggerSource::DriftObservation { card }
