...
//!
//! `TlsConfig` and `SecretRef` are consumed by the external client
//! (`wyrd-client::transport`). Keeping them in `wyrd-spec::security` gives
//! them one durable, schema-bearing definition without a cross-crate
//! dependency cycle.
