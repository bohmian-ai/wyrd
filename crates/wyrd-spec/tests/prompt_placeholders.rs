...
use wyrd_spec::extract_placeholders;
...
fn extracts_placeholder_in_anthropic_system_text() {
    ...
    assert_eq!(
        extract_placeholders(&spec).expect("extracts"),
        vec!["topic"]
    );
}
...
fn extracts_placeholder_in_user_message_text() {
    ...
    assert_eq!(extract_placeholders(&spec).expect("extracts"), vec!["name"]);
}
...
fn extracts_placeholder_in_tool_input_and_result_strings() {
    ...
    assert_eq!(extract_placeholders(&spec).expect("extracts"), vec!["city"]);
}
...
fn extracts_first_seen_order_stable_and_deduplicated() {
    ...
    assert_eq!(
        extract_placeholders(&spec).expect("extracts again"),
        vec!["first", "second"]
    );
}
