...
use wyrd_auth_oidc::{JwksCache, TrustedIssuerRegistry};
...
/// key is sufficient. When a resolved [`TrustedIssuerRegistry`] is supplied, the
/// verifier's external (foreign-OIDC) path is wired to it so federated tokens
/// can be exchanged.
///
/// # Errors
...
pub fn build_auth_handles(
...
) -> Result<(Arc<IssuingKey>, Arc<TokenVerifier<SqlPermissionResolver>>), ServerBootError> {
    ...
    let verifier = match trusted_issuer_registry {
...
mod tests {
    ...
    async fn build_auth_handles_mints_tokens_verifiable_by_the_derived_key() {
        let (issuing_key, _verifier) =
            build_auth_handles(&SecretString::from(PRIVATE_KEY_PEM), &lazy_pool(), None)
    ...
    async fn build_auth_handles_rejects_an_invalid_signing_key() {
        let result = build_auth_handles(&SecretString::from("not a pem"), &lazy_pool(), None);
