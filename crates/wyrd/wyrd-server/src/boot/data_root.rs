...
//! no local path.
//!
//! Preparation creates every managed path and takes an exclusive advisory lock
//! on the root before any role is constructed, so a second process mounting
//! the same root fails boot instead of reusing another replica's WAL identity.
...
pub enum BifrostDataRootError {
    /// A managed path could not be created or the lock file could not be opened or locked.
...
impl BifrostDataRoot {
    /// Creates every managed path below `root` and locks the root exclusively.
    ///
    ...
    /// contents are never removed here.
    ///
    /// # Errors
    ///
    /// Returns [`BifrostDataRootError::Unusable`] when a managed directory or
    /// the lock file cannot be created or locked, and
    ...
    pub fn prepare(root: &Path) -> Result<Self, BifrostDataRootError> {
        ...
        match lock.try_lock() {
            Ok(()) => Ok(Self { roots, _lock: lock }),
            Err(TryLockError::WouldBlock) => Err(BifrostDataRootError::InUse {
                path: root.to_path_buf(),
            }),
            Err(TryLockError::Error(source)) => Err(BifrostDataRootError::Unusable {
                path: lock_path,
                source,
            }),
        }
