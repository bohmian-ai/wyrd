...
//!
//! Composed into the authenticated `/v1` group beside
//! `crate::storage::routes::router`. The `/v1` group has **no structural auth
...
/// Standalone eval pull-protocol router for the `/v1` group.
///
/// Mirrors `crate::storage::routes::router`. Every route resolves the principal
