...
mod tests {
    ...
    /// The router publishes the OpenAPI document as JSON and nothing else.
    ///
    /// Wyrd had two copies of one contract: this route pair, plus a checked-in
    /// `openapi.yaml` regenerated by a separate lane. Two artifacts of the same
    /// document can disagree, and the one a caller reads is the one that is
    /// wrong. `GET /openapi.json` is now the whole contract.
    ///
    /// Route registration is asserted against this module's own source because
    /// `axum::Router` exposes no route table and building the real one needs a
    /// live `AppState` — the same reason `exchange_api_key`'s blocking-pool test
    /// reads its source. The document itself is built and serialized here, so a
    /// handler whose schema cannot render still fails this test.
    #[test]
    fn only_json_openapi_is_served() {
        let source = include_str!("router.rs");
        let registrations = source.matches(".route(\n            \"/openapi").count();

        assert_eq!(
            registrations, 1,
            "exactly one OpenAPI route is registered on the router"
        );
        assert!(
            source.contains("\"/openapi.json\","),
            "GET /openapi.json serves the generated document"
        );
        assert!(
            !source.contains("\"/openapi.yaml\","),
