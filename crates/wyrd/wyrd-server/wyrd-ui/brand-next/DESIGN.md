...
- Regenerate `theme.css` after any token edit: `node brand-next/gen-theme.mjs`. Drift is
  checked with `--check`.
...
| `--font-display` | Space Grotesk 700 | headings, KPI numerals, sidebar brand |
| `--font-mono` | JetBrains Mono | **all data** — tables, badges, IDs, code, nav, metrics |
| `--font-serif` | Fraunces | **loud altitude and marketing only** |

`--font-serif` never appears in the workbench. Fraunces is the company voice — hero
display, landing pages, the wordmark — and mixing it into quiet altitude collapses the
distinction between "this is bohmian talking" and "this is your system reporting".
