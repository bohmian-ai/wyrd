...
- `palette.json` — canonical tokens → generates `theme.css` (`node brand-next/gen-theme.mjs`)
...
```sh
cd crates/wyrd/wyrd-server/wyrd-ui/brand-next && python3 -m http.server 8777
