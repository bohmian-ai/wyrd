#!/usr/bin/env node
// Generates brand-next/theme.css from brand-next/palette.json (the canonical
// token source).
//
// Usage:
//   node brand-next/gen-theme.mjs           regenerate theme.css
//   node brand-next/gen-theme.mjs --check   exit non-zero if theme.css is stale
//
// Unlike brand/gen-theme.mjs, this emits a SINGLE target. brand-next/ is inert:
// it must not write into .claude/skills, .agents/skills, or docs/src/styles
// while the live theme is still brand/. Those emitters return at cutover.
...
const banner = [
  ...
  ' * Source of truth: brand-next/palette.json.',
  ' * Regenerate with `node brand-next/gen-theme.mjs`.',
...
if (process.argv.includes('--check')) {
  ...
  for (const { path, content } of targets) {
    ...
    if (existing !== content) {
      console.error(`${path} is out of date with palette.json. Run \`node brand-next/gen-theme.mjs\`.`);
      allOk = false;
    }
  }
  if (!allOk) {
