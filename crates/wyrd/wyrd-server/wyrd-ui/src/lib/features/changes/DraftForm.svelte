<script lang="ts">
  ...
  let intentOpen = $state(true);
  let claimsOpen = $state(true);
  $effect(() => {
    intentOpen = !narrow.current;
    claimsOpen = !narrow.current;
  });
...
<form method="POST" action="?/save" class="stack draft" use:enhance={() => { busy = true; return async ({ update }) => { await update({ reset: false }); busy = false; }; }}>
  ...
  <div class="columns"><div class="stack">
    <Panel title="1 · What and why"><div class="stack"><label>Title<input name="title" bind:value={draft.title} maxlength="300" /></label><details bind:open={intentOpen}><summary>Intent and impact</summary><div class="stack"><label>Intent<textarea name="intent" bind:value={draft.intent} rows="2"></textarea></label><label>Impact<input name="impact" bind:value={draft.impact} /></label></div></details></div></Panel>
