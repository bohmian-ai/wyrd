<script lang="ts">
  ...
  import type { ChangeView, ActionResult, Anchor } from './types';
  let {
    view,
    csrf,
    base,
  ...
  function anchorLink(anchor: Anchor): string {
    if (anchor.kind === 'source' || anchor.kind === 'subject')
      return `${base}/subjects/${anchor.target}?revision=${anchor.revision}${anchor.file ? `&file=${encodeURIComponent(anchor.file)}` : ''}${anchor.line ? `#line-${anchor.line}` : ''}`;
...
<div class="columns review"><div class="stack">
  ...
  {#each change.threads as thread}<section id={thread.id}><Panel title={`Thread · ${thread.anchor.kind} · ${thread.anchor.file ?? thread.anchor.target}${thread.anchor.line ? ':' + thread.anchor.line : ''}`}>{#snippet head()}<a href={anchorLink(thread.anchor)}>Anchor →</a><Badge tone={thread.resolved ? 'ok' : 'running'}>{thread.resolved ? 'Resolved' : 'Open'}</Badge>{/snippet}
...
</div><aside class="stack"><section id="submit-review" class="raised"><Panel title={`Submit review — revision ${change.revisionNumber}`} variant="raised"><form method="POST" action="?/review" class="stack review-form" use:enhance><input type="hidden" name="csrf" value={csrf} /><input type="hidden" name="revision" value={change.revision} /><input type="hidden" name="requestKey" value={`${view.requestKey}:decision`} /><input type="hidden" name="operation" value="decision" /><label>Optional summary<textarea name="body" maxlength="10000"></textarea></label><fieldset class="stack"><legend>Review decision</legend>{#each [['comment','Comment','Feedback without a decision'],['approve','Approve','Records approval — never verifies or merges'],['request-changes','Request changes','Blocks until a new revision addresses feedback']] as [value,title,detail]}<label class="decision"><input type="radio" name="decision" {value} checked={value === 'comment'} /><span><strong>{title}</strong><p>{detail}</p></span></label>{/each}</fieldset><button class="control primary" disabled={!view.capabilities.review}>Submit review</button><small>Reviews exactly revision {change.revisionNumber}. A new revision requires re-review.</small></form></Panel></section>
