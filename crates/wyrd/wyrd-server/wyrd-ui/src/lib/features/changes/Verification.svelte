...
<div class="stack verification">
  ...
  <Panel title="Claims">{#snippet head()}<span class="mono muted">{change.total} required</span>{/snippet}
    ...
    {#each claim.checks as check}<div class="check" id={`check-${check.id}`}>
      ...
      <div class="check-actions"><span class="badges"><Badge tone={check.summary.tone}>{check.summary.label}</Badge>{#if check.provenance !== 'current'}<Badge tone={check.provenance === 'stale' ? 'warn' : 'neutral'}>{check.provenance.replaceAll('_',' ')}</Badge>{/if}</span>
      {#if check.eligible && view.capabilities.run}<button class="control" type="button" onclick={() => selected = selected === check.id ? '' : check.id}>{check.action}</button>{/if}
  ...
  {#if view.capabilities.write}<details class="amend"><summary>Amend Claims — creates revision {change.revisionNumber + 1}</summary>
    <form method="POST" action="?/revise" class="stack divider" use:enhance>
      ...
      <button class="control">Create revision {change.revisionNumber + 1}</button>
