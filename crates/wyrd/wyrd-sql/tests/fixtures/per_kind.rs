...
use wyrd_spec::vala::eval::EvalContextCapture;
...
fn minimal_drift_spec() -> DriftSpec {
    DriftSpec::new(
        DriftMethod::External,
        card_ref(CardKind::Model, "drift-subject"),
        DriftSignal::External {
            source_ref: card_ref(CardKind::Data, "drift-source"),
        },
...
pub fn minimal_spec(kind: CardKind) -> Spec {
    match kind {
        ...
        CardKind::Eval => Spec::Eval(EvalSpec::new(BTreeMap::new()).expect("fixture eval spec")),
        CardKind::Drift => Spec::Drift(minimal_drift_spec()),
...
pub fn mutate_for_drift(card: &mut Card) {
    match &mut card.spec {
        ...
        Spec::Eval(s) => s.context_capture = Some(EvalContextCapture::Redact),
