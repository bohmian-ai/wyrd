...
//!
//! Env-gated by `WYRD_REG_E2E=1`. Run via `mise run test:sql`.
//!
...
mod pg_tests {
    ...
    use crate::fixtures::{
        TestEnv, asserts::*, fixture_card, fixture_card_auto, fixture_card_scoped, per_kind,
        scenarios, with_annotations, with_bump, with_labels,
    };
    ...
    e2e_test!(auto_register_resolves_deduplicates_and_audits_outcomes, {
        ...
        per_kind::mutate_for_drift(&mut changed);
    ...
    e2e_test!(
        card_register_reregister_delete_write_one_typed_outbox_detail,
        {
            let env = TestEnv::new().await;
            let tenant = env.fresh_tenant().await;
            let actor = env.fixture_user(tenant).await;
    ...
    e2e_test!(query_cards_version_range_and_prerelease_predicates, {
        ...
        let in_range_versions: Vec<&str> =
            in_range.items.iter().map(|r| r.version.as_str()).collect();
        assert!(
