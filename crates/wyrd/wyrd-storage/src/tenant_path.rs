...
#[cfg(test)]
mod tests {
    use super::{build, strip_bucket, validate};
    use wyrd_spec::DataTenantId;
    ...
    fn strip_bucket_returns_tenant_prefixed_path_for_supported_schemes() {
        let tenant = DataTenantId::new_v7();
        let s3 =
            url::Url::parse(&format!("s3://bucket/{tenant}/cards/x/y.parquet")).expect("s3 uri");
        let gs =
            url::Url::parse(&format!("gs://bucket/{tenant}/cards/x/y.parquet")).expect("gs uri");
        let file = url::Url::parse(&format!("file://localhost/{tenant}/cards/x/y.parquet"))
            .expect("file uri");
    ...
    fn strip_bucket_rejects_unknown_uri_scheme() {
        let uri =
            url::Url::parse("http://example.test/tenant/cards/x/y.parquet").expect("http uri");

        let error = strip_bucket("http", &uri).expect_err("unsupported scheme should fail");

        assert!(matches!(error, crate::StorageError::InvalidUri(_)));
    }
