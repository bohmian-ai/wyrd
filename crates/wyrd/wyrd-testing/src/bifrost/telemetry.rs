...
mod tests {
    ...
    /// Map one changed production counter delta without changing any peer field.
    #[test]
    fn forge_telemetry_report_uses_production_delta() {
        ...
        let mut changed = complete_delta();
        changed.metrics[0].value *= 2.0;
        let changed = ForgeMaintenanceTelemetryReport::from_production_delta(&changed, &expected)
            .expect("changed production delta maps");
        assert_eq!(baseline.throughput_mib_per_sec, 0.5);
        assert_eq!(changed.throughput_mib_per_sec, 1.0);
        assert_eq!(baseline.task_latency_p99_us, changed.task_latency_p99_us);
        assert_eq!(baseline.role_topology, changed.role_topology);
    }
    ...
    /// Pin every production family and exact span name in the owning source modules.
    #[test]
    fn forge_production_telemetry_contract() {
        let manifest = std::path::Path::new(env!("CARGO_MANIFEST_DIR"));
        let forge_root = manifest.join("../../vala/vala-bifrost-redux/src/forge");
        let sources = [
            forge_root.join("metrics.rs"),
            forge_root.join("mod.rs"),
            forge_root.join("planning_scheduler.rs"),
            forge_root.join("scheduler.rs"),
            forge_root.join("worker.rs"),
            forge_root.join("compact.rs"),
            forge_root.join("live_replace.rs"),
            manifest.join("../../vala/vala-bifrost-redux/src/scribe/memory.rs"),
        ]
        .into_iter()
        .map(|path| std::fs::read_to_string(path).expect("production Forge source is readable"))
        .collect::<String>();
        for family in [
            "bifrost_forge_rewrite_output_bytes_total",
            "bifrost_forge_task_duration_seconds",
            ...
            assert!(
                sources.contains(family),
                "missing production family {family}"
            );
        }
        for span in [
            "bifrost.forge.scheduler.pass",
            ...
            assert!(sources.contains(span), "missing production span {span}");
        }
    }

    /// Reject benchmark-local timing and storage inspection as report inputs.
    #[test]
    fn forge_benchmark_has_no_parallel_metric_derivation() {
        ...
        assert!(!source.contains("query_latency"));
        assert!(!source.contains("schedule_once"));
        assert!(!source.contains("execute_one_for_test"));
        assert!(source.contains("worker.run"));
        assert!(source.contains("forge.run"));
        assert!(source.contains("ForgeMaintenanceTelemetryReport::from_production_delta"));
    }

    /// Preserve active concurrency while retaining the fourth replacement start.
    #[test]
    fn forge_role_topology_survives_worker_replacement() {
        let expected = BTreeMap::from([("server".to_owned(), 1), ("forge_worker".to_owned(), 3)]);
        let report =
            ForgeMaintenanceTelemetryReport::from_production_delta(&complete_delta(), &expected)
                .expect("replacement topology maps");
        let workers = &report.role_topology["forge_worker"];
        assert_eq!(workers.max_active, 3);
        assert_eq!(workers.final_active, 3);
        assert_eq!(workers.starts, 4);

        let mut wrong_maximum = complete_delta();
        wrong_maximum
            .gauge_maxima
            .iter_mut()
            .find(|sample| {
                sample.family == "bifrost_forge_role_processes"
                    && sample.labels.get("role").map(String::as_str) == Some("forge_worker")
            })
            .expect("worker maximum series")
            .value = 4.0;
        assert_eq!(
            ForgeMaintenanceTelemetryReport::from_production_delta(&wrong_maximum, &expected),
            Err(ForgeTelemetryReportError::TopologyMismatch)
        );

        let mut wrong_final = complete_delta();
        wrong_final
            .gauge_final
            .iter_mut()
            .find(|sample| {
            ...
            .expect("worker final series")
            .value = 2.0;
        assert_eq!(
            ForgeMaintenanceTelemetryReport::from_production_delta(&wrong_final, &expected),
            Err(ForgeTelemetryReportError::TopologyMismatch)
        );
    }

    /// Derive rewrite throughput only from production bytes and window duration.
    #[test]
