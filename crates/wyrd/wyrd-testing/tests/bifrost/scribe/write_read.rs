...
/// The canonical production run reconciles registered starts and terminals.
///
/// # Panics
///
/// Panics when the production workload fails, an activity is unbalanced, or
/// publication completes without upload and fenced file-list emissions.
#[tokio::test]
#[ignore = "requires Postgres and object storage"]
async fn scribe_hot_path_telemetry_reconciles() {
    ...
    server
        .run_scribe_production_workload(&workload, ScribeCacheMode::Disabled)
        .await
        .expect("canonical production workload");
    let snapshot = server
        .bifrost_scribe()
        .expect("production Scribe owner")
        .hot_path_telemetry_for_test();
    assert_eq!(snapshot.active, 0, "all registered activities must settle");
    assert_balanced(&snapshot.effects, "admission.started", "admission.settled");
    assert_balanced(&snapshot.effects, "claim.started", "claim.settled");
    assert_balanced(&snapshot.effects, "queue.entered", "schedule.settled");
    assert_balanced(&snapshot.effects, "upload.started", "upload.settled");
    assert_balanced(&snapshot.effects, "file_list.started", "file_list.settled");
    assert_balanced(&snapshot.effects, "cleanup.started", "cleanup.settled");
    assert_balanced(
        &snapshot.effects,
        "wal_append.started",
        "wal_append.settled",
    );
    assert_balanced(&snapshot.effects, "wal_fsync.started", "wal_fsync.settled");
    assert_eq!(
        snapshot.effects["schedule.started"], snapshot.effects["schedule.settled"],
        "every scheduled persistence job reaches one terminal"
    );
    assert!(snapshot.effects["route.shard_selected"] > 0);
    assert!(snapshot.effects["rotation.rotated"] > 0);
    assert!(snapshot.effects["freeze.frozen"] > 0);
    assert!(snapshot.effects["run.durable"] > 0);
    assert!(snapshot.effects["manifest.durable"] > 0);
    assert!(snapshot.effects["source.transitioned"] > 0);
    assert!(snapshot.effects["ready.member_ready"] > 0);
    assert!(snapshot.effects["merge.pass_completed"] > 0);
    assert!(snapshot.effects["row_group.flushed"] > 0);
    let closed =
        snapshot.artifacts["object_close.target"] + snapshot.artifacts["object_close.residue"];
    assert!(closed > 0, "production workload closed no physical object");
    assert_eq!(
        snapshot.artifacts["upload.settled"], closed,
        "one verified PUT must settle for every closed object"
    );
    assert_eq!(
        snapshot.artifacts["file_list.settled"], closed,
        "one fenced file-list row must settle for every closed object"
    );
    server.shutdown().await.expect("the server drains cleanly");
}

/// Requires a nonzero start count exactly equal to its terminal count.
fn assert_balanced(effects: &std::collections::BTreeMap<String, u64>, start: &str, terminal: &str) {
    assert!(effects[start] > 0, "{start} has no production emitter");
    assert_eq!(effects[start], effects[terminal], "{start} did not balance");
}
