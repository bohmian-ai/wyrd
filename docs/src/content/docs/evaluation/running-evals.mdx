...
comparison engine against the current run. Local flows write `results.json`, and, when a baseline is present,
`comparison.json` under `--out` or `.wyrd/eval/<run_id>/`.
...
`EvalResults` payload. `comparison.json` is present only when `--baseline` is
