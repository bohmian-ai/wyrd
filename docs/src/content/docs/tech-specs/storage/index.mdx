...
| L15 | Server-owned or server-verified completion | S3 and Local finalize server-side with SHA-256 enforcement; GCS and Azure are client-declared (see [Storage setup](/setup/storage/#sha-256-integrity-guarantees)). |
| L17 | SHA-256 recorded before `mark_completed` | S3: backend-enforced at commit. Local: server-computed from written bytes. GCS/Azure: client-declared at init, recorded in artifact metadata and audit ledger. |
...
| GCS | `GcsResumable { session_uri }` | Chunked PUT | Client finalizes; server records client-declared SHA-256 | Client-declared (GCS stores CRC32c/MD5 only) |
