...
# WYRD_TEST_PACKAGES, when set, is the space-separated affected-package set CI
# selected; the family then tests only its members in that set.
set -euo pipefail
...
for pkg in "${packages[@]}"; do
  if [[ -z "${WYRD_TEST_PACKAGES+set}" || " $WYRD_TEST_PACKAGES " == *" $pkg "* ]]; then
    cargo_args+=("-p" "$pkg")
  fi
done
